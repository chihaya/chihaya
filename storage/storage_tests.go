...
func TestPeerStore(t *testing.T, p PeerStore) {
	testData := []struct {
	...
	}{
		{
			...
			bittorrent.Peer{
				ID:       bittorrent.PeerIDFromBytes([]byte("00000000000000000001")),
		...
		},
		{
			...
			bittorrent.Peer{
				ID:       bittorrent.PeerIDFromBytes([]byte("00000000000000000002")),
				AddrPort: netip.MustParseAddrPort("[abab::0001]:2"),
			},
		},
	}

	v4Peer := bittorrent.Peer{
		ID:       bittorrent.PeerIDFromBytes([]byte("99999999999999999994")),
		AddrPort: netip.MustParseAddrPort("99.99.99.99:9994"),
	}
	v6Peer := bittorrent.Peer{
		ID:       bittorrent.PeerIDFromBytes([]byte("99999999999999999996")),
		AddrPort: netip.MustParseAddrPort("[fc00::0001]:9996"),
	}

	for _, c := range testData {
		peer := v4Peer
		if c.peer.AddrPort.Addr().Is6() {
			peer = v6Peer
		}
		...
		scrape := p.ScrapeSwarm(c.ih, c.peer)
